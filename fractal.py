...
iterations = 10         # Number of times of iterative transformations, positive integer
span = 50               # Extent of spiral rendering (in radians), positive real
k = 0.8                 # Defines polar slope of the spiral (the greater the value of k, the less the slope), real in the interval (0, 1) 
lnk = ln(k)             # Natural logarithm of k
downscale = 3/k         # Downscaling factor of size of spirals between successive iterations
width, height = 800, 800                                            # Width and height of final output image in pixels
img_scaling = 1.5                                                   # Upscaling factor to generate size of image rendering canvas
...
for point in range(num_points) :
    ...
    y = round(z.imag * resy/4 + resy/2)     # | Draw the point 
    img.putpixel((x, y), (255,255,255))     # |

    if not (point/num_points*100)%1 : print(f'{(point*100)//num_points+1}% completed')      # Progress update
